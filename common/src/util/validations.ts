...
export const Validation = {
    usernameRegExp: /^[\w.@+-]+$/,
    usernameInvalidMsg: "Username may contain only letters, numbers, and @/./+/-/_ characters.",
    phoneRegExp: /([0-9\s\-?]{7,})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(\d+))?$/,
    emailRegExp:
        /^([a-zA-Z0-9]+(?:[.-]?[a-zA-Z0-9]+)*@[a-zA-Z0-9]+(?:[.-]?[a-zA-Z0-9]+)*\.[a-zA-Z]{2,7})$/,
    passwordRegExp: /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.{8,})/,
    passwordInvalidMsg:
        "Password must be at least 8 characters long and must contain at least: one lowercase letter, one uppercase letter and one number.",
