"""
Taken from https://github.com/jazzband/django-downloadview/blob/2.1.1/django_downloadview/views/object.py#L8
...
class AuthenticatedObjectDownloadView(SingleObjectMixin, AuthenticatedBaseDownloadView):
    ...
    * :attr:`size_field`.
    ...
    def get_basename(self):
        ...
        basename = super(AuthenticatedObjectDownloadView, self).get_basename()
        if basename is None:
            field = "basename"
            model_field = getattr(self, "%s_field" % field, False)
            if model_field:
